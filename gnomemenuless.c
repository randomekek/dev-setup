/* gcc -shared `pkg-config gtk+-x11-2.0 --cflags --libs` -fPIC -o gnomemenuless.so gnomemenuless.c */
/* GTK_MODULES=gnomemenuless.so geany */
...
static void
toggle_menu_bar (GtkWidget *widget, gpointer user_data)
{
...
static void
anchor_event (GtkWidget *widget, GtkWidget *previous_toplevel, gpointer user_data)
{
	GtkWidget *toplevel = gtk_widget_get_toplevel (widget);
	if (GTK_WIDGET_TOPLEVEL(toplevel)) {
		if (previous_toplevel) {
			g_signal_handlers_disconnect_by_func(previous_toplevel,
					G_CALLBACK(toggle_menu_bar), widget);
		}
...
GObject*
new_gtk_menu_bar_constructor (GType type, guint n_construct_properties,
...
gtk_module_init (gint * argc, gchar *** argv)
{
	GtkWidget *fc;
	GObjectClass *klass;
	const gchar *app_whitelist = "avidemux2_gtk, gvim";

	if (strstr (app_whitelist, g_get_prgname()) != NULL) {
		return;
	}

	g_signal_new ("toggle-menu-bar", GTK_TYPE_WINDOW, G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, 0, NULL, NULL, _gtk_marshal_VOID__VOID, G_TYPE_NONE, 0);
    ...
    // makes alt+m show and hide the menu
	gtk_binding_entry_add_signal (gtk_binding_set_by_class (klass), GDK_m, GDK_CONTROL_MASK, "toggle-menu-bar", 0);
	gtk_binding_entry_add_signal (gtk_binding_set_by_class (klass), GDK_m, GDK_MOD1_MASK, "toggle-menu-bar", 0);
}
